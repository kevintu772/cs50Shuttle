...
function dropoff()
{
    for (var i = 0; i < SEATS; i++)
    {
        if (shuttle.seats[i] != null)
        {
            ...
            var long = HOUSES[house].lng;
            var d = shuttle.distance(lat, long);
            
            if (d <= 30)
            {
                shuttle.seats[i] = null;
                $("#announcements").html("Student successfully dropped off"); 
                  
                for (var j = i; j < SEATS; j++)
                {
                    if (shuttle.seats[j+1] != null)
                    {
                        shuttle.seats[j] = shuttle.seats[j+1];
                        shuttle.seats[j+1] = null;
                    }
                }
            }
            else
            {
                $("#announcements").html("No target house within 30 meters of the Shuttle");    
            }
        }
    }
    chart()
...
/**
 * Returns true if passenger is a freshman, else false
 */

function freshman(house)
{
    var houses = ["Adams House",
    "Cabot House",
    "Currier House",
    "Dunster House",
    "Eliot House",
...
{   
    ...
    {
        if ((PASSENGERS[i].placemarker != 0) && (PASSENGERS[i].marker != 0))
        {
            var lat = PASSENGERS[i].placemarker.getGeometry().getLatitude();
            var long = PASSENGERS[i].placemarker.getGeometry().getLongitude();
            var d = shuttle.distance(lat, long);
            var empty_seats = 0;
        
        
            if ((d <= 15) && (freshman(PASSENGERS[i].house) == false))
            {
                for (var j = 0; j < SEATS; j++)
                {
                    if (shuttle.seats[j] == null)
                    {
                        ...
                        var student = PASSENGERS[i].placemarker;
                        var label = PASSENGERS[i].marker;
                        
                        features.removeChild(student);
                        label.setMap(null);
                        PASSENGERS[i].placemarker = 0;
                        PASSENGERS[i].marker = 0;
                        
                        shuttle.seats[j] = PASSENGERS[i];
                        $("#announcements").html("Student onboard!");
            ...
            /*else if (freshman(PASSENGERS[i].house) == true)
            {
                $("#announcements").html("No freshmen allowed onboard");
            }*/
            else if (found == false)
            {
                $("#announcements").html("No students found within 15 meters of the Shuttle");
            }
        }   
    }
    chart();
...
function populate()
{
    ...
    for (var i = 0; i < PASSENGERS.length; i++)
    {
        ...
        PASSENGERS[i].placemarker = placemark;
        PASSENGERS[i].marker = marker;
        PASSENGERS[i].location = building;
    }
}
